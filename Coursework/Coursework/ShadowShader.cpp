...
void ShadowShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	// description to setup light buffer 
...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMap, ID3D11ShaderResourceView* depthMap2, Light* dirLight, Light* spotLight, Light* pointLight,float spotAngle)
{
	...
	XMMATRIX dirLightView = XMMatrixTranspose(dirLight->getViewMatrix());
	XMMATRIX dirLightProj = XMMatrixTranspose(dirLight->getOrthoMatrix());

	XMMATRIX spotV = XMMatrixTranspose(spotLight->getViewMatrix());
	XMMATRIX spotProj = XMMatrixTranspose(spotLight->getOrthoMatrix());
	...
	dataPtr->dirView = dirLightView;
	dataPtr->dirProj = dirLightProj;
	dataPtr->spotView = spotV;
	dataPtr->spotProj = spotProj;
